<template>
  <view class="reset-psd">
    ...
    <form @submit="submit" class="form">
      <view class="item">
        <text class="text">当前密码</text>
        <input type="text" name="oldPsd" v-model="oldPsd" placeholder="填写原密码" @blur="checkOldPsd"/>
      </view>
      <view class="item">
      ...
      <navigator url="findPsd" class="forget">
        忘记密码？
      </navigator>
    </form>
  </view>
    ...
    methods: {
      ...
      submit: function (e) {
        ...
        if (!e.detail.value.oldPsd || !e.detail.value.newPsd || !e.detail.value.surePsd) {
          ...
          uni.request({
            url: urls.HTTP + '/user/editpassword',
            method: 'POST',
            data: {
              id: id,
